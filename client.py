...
def register(username, password):
    ...
    response = requests.post('http://localhost:8000/api/register/', data=json.dumps(credentials), headers=header_login)
    try:
...
def login(username, password):
    header_login = {'Content-type': 'application/json'}
    credentials = {'username': username, 'password': password}
    response = requests.post('http://localhost:8000/api/login/', data=json.dumps(credentials), headers=header_login)
    try:
        response.raise_for_status()
...
def send_message(auth_token, username_dest, message):
    ...
    response = requests.post('http://localhost:8000/api/messages/', data=json.dumps(message_request), headers=header_message)
    try:
        response.raise_for_status()
...
def get_messages(auth_token):
    header_message = {'Content-type': 'application/json', 'Authorization': f'Token {auth_token}'}
    response = requests.get('http://localhost:8000/api/messages/', headers=header_message) #doc din elastisearch
    try:
        response.raise_for_status()
